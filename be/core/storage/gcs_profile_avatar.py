"""Upload profile avatar images to GCS."""
...
def upload_profile_avatar_image(*, user_id: uuid.UUID, content_type: str, data: bytes) -> str:
    """
    Upload bytes to ``{user_id}/{cuid}.{ext}`` in the profile avatars bucket.
    ...
    """
    bucket_name = config.gcs_profile_avatars_bucket.strip()
    if not bucket_name:
        raise ValueError("GCS profile avatars bucket is not configured")
    ...
    object_name = f"{user_id}/{Cuid(length=16).generate()}{suffix}"
